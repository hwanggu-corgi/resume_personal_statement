...
저는 SiteMax System에서 일하면서 저녁에 Team Treehouse에서 공부했고 Django 기반 Full Stack Development를
배우며 큰 앱을 만들면서 수정을 할때 아니면 새로운것을 만들때 늘 기존에 잘 동작된것들이 동작이 잘 안되는 것을 최소화 하는데
도움주는 Test Driven 개발법을 습득하였습니다.

\bigskip

저는 지금 한층 더 나아가 토론토 대학에서 여름때 배운 C 언어와 함께 OS Programming을 배우며 요구사항을에 맞추어 프로그램을 구축하는 스킬을 hone 하고 있습니다.

\bigskip

큰 앱을 만들때 가장 어려운 것중에 하나가 사전 준비 없이 앱을 만들때 생기는 문제입니다.
저는 토론토 대학에서 C 언어와 shell scripting을 함께 이용해 FUSE기반 파일 시스템을 무 에서 유로 만들때 안정적
구축에 필요한 큰 아이디어를 proposal에 썼고 아이디어가 요구사항에 완벽하진 않지만 가까스럽게 맞았을때 실행으로 옮겼습니다.
그리고 만들때는 C 언어와 shell script를 이용한 TDD를 사용하였고 그
