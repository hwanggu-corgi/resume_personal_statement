...
저는 지금 토론토 대학에서 여름때 배운 C 언어와 함께 OS Programming을 배우며 요구사항을에 맞추어 프로그램을 구축하는 스킬을 hone 하고 있습니다.

\bigskip

큰 앱을 만들때 가장 어려운 것중에 하나가 많은것을 만들었을때 하나의 문제로 안된다거나 기존에 잘 동작된것들이 수정 후 동작이 잘 안되는것입니다.
토론토 대학에서 C 언어와 shell scripting을 함께 이용해 FUSE기반 파일 시스템을 무 에서 유로 만들때 안정적
구축에 필요한 큰 아이디어를 proposal에 썼고 아이디어가 어느정도 구축되었을때 실행으로 옮겼습니다.
